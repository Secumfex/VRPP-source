...
void RenderloopPlaceHolderListener::update() {
	...
	rm->setCurrentFBO(fbo); //@TODO evtl. fbo binden ....
...
GuiLoopListener::GuiLoopListener() {
	rm = RenderManager::getInstance();
	gbo = new FrameBufferObject();
}

void GuiLoopListener::update() {
	glEnable(GL_DEPTH_TEST);
	glClear(GL_DEPTH_BUFFER_BIT);
	glViewport(0, 0, 800, 600);

	rm->setCurrentFBO(gbo);
	...
	currentRenderQueue = rm->getRenderQueue(); //dont know if needed
...
SetClearColorListener::SetClearColorListener(float r, float g, float b,
		float a) {
	this->r = r;
	this->g = g;
	this->b = b;
...
TurnCameraListener::TurnCameraListener(Camera* cam, float delta_phi,
		float delta_theta) {
	this->cam = cam;
...
ApplyForceOnSelectedPhysicsComponentInCameraViewDirectionListener::ApplyForceOnSelectedPhysicsComponentInCameraViewDirectionListener(
		SelectionHandler* selectionHandler, Camera* cam, float strength) {
...
void ApplyForceOnSelectedPhysicsComponentInCameraViewDirectionListener::update() {
	if (selectionHandler->somethingIsSelected()) {
		/*Pray and Cast*/
		void* userPointer =
				selectionHandler->getCurrentSelection()->getUserPointer();
		if (userPointer != 0) {
			btRigidBody* rigidBody =
					(static_cast<PhysicsComponent*>(userPointer))->getRigidBody();
...
UpdateVirtualObjectModelMatrixListener::UpdateVirtualObjectModelMatrixListener(
		VirtualObject* vo) {
	this->vo = vo;
}
...
ApplyLinearImpulseOnRigidBody::ApplyLinearImpulseOnRigidBody(
		btRigidBody* rigidBody, btVector3 force) {
...
SetCameraPositionListener::SetCameraPositionListener(Camera* cam,
		glm::vec3 position) {
	this->cam = cam;
